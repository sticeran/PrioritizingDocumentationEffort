...
#     dataFrame_dataset = dataFrame_dataset[dataFrame_dataset['avgUserScore']>0].sort_values(by = 'avgUserScore',axis = 0,ascending = False).reset_index(drop=True);
    #只得到第一列数据，即类全名
    list_column1 = dataFrame_dataset['File Name'];
...
#     dataFrame_dataset = dataFrame_dataset[dataFrame_dataset['avgUserScore']>0].sort_values(by = 'avgUserScore',axis = 0,ascending = False).reset_index(drop=True);
    for i in range(len(dataFrame_dataset)):
        dataFrame_dataset.ix[i,'File Name'] =dataFrame_dataset.ix[i,'File Name'].replace("\\", "/");
    df_intersection=pd.merge(dataFrame_dataset,dataFrame_fileList,on='File Name');
